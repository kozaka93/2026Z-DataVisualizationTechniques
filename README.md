# Techniki wizualizacji danych

Semestr zimowy 2026/27 
Materiały z zajęć *Techniki wizualizacji danych* prowadzonych na Wydziale Matematyki i Nauk Informacyjnych Politechniki Warszawskiej.

Techniki wizualizacji danych składają się z:

-   wykładu [@kozaka93](https://github.com/kozaka93)
-   zajęć laboratoryjnych [@kozaka93](https://github.com/kozaka93) [@xxkaro](https://github.com/xxkaro) [@xx](https://github.com) [@yy](https://github.com) [@woznicak](https://github.com/woznicak)
-   zajęć projektowych [@kozaka93](https://github.com/kozaka93) [@danielewskai](https://github.com/danielewskai) [@xx](https://github.com/) [@woznicak](https://github.com/woznicak)

### Terminy i tematy zajęć

<table style="undefined;table-layout: fixed; width: 863px"><colgroup>
<col style="width: 79px">
<col style="width: 69px">
<col style="width: 75px">
<col style="width: 68px">
<col style="width: 482px">
<col style="width: 90px">
</colgroup>
<thead>
  <tr>
    <th>DZIEŃ</th>
    <th>TYDZIEŃ</th>
    <th>GODZINY</th>
    <th>ZAJĘCIA</th>
    <th>TEMAT</th>
    <th>SKŁADOWA</th>
  </tr></thead>
<tbody>
  <tr>
    <td rowspan="2"></td>
    <td rowspan="2">1</td>
    <td>2</td>
    <td>wykład</td>
    <td>Wstęp, zasady zaliczenia, eksploracja danych, narzędzia, przedstawienie tematyki projektów</td>
    <td></td>
  </tr>
  <tr>
    <td>2</td>
    <td>lab</td>
    <td>R: wstęp, powtórka R, wstęp dplyr</td>
    <td></td>
  </tr>
  <tr>
    <td rowspan="2"></td>
    <td rowspan="2">2</td>
    <td>2</td>
    <td>wykład</td>
    <td>Sposoby badania rozkładu jednej zmiennej (ilościowej i jakościowej), sposoby badania rozkładu dwóch i więcej zmiennych</td>
    <td></td>
  </tr>
  <tr>
    <td>2</td>
    <td>lab</td>
    <td>R: dplyr, tidyr, idea ggplot2</td>
    <td></td>
  </tr>
  <tr>
    <td rowspan="2"></td>
    <td rowspan="2">3</td>
    <td>2</td>
    <td>projekt</td>
    <td>P1: pomysł, źródło danych</td>
    <td>P1 (1p)</td>
  </tr>
  <tr>
    <td>2</td>
    <td>lab</td>
    <td>R: ggplot2 - badanie rozkładu jednej zmiennej, wielu zmiennych</td>
    <td></td>
  </tr>
  <tr>
    <td rowspan="2"></td>
    <td rowspan="2">4</td>
    <td>2</td>
    <td>wykład</td>
    <td>Kolory i skale, mapy - czy to takie skomplikowane?</td>
    <td></td>
  </tr>
  <tr>
    <td>2</td>
    <td>lab</td>
    <td>R: ggplot2 - skale, osie, kolor, legenda</td>
    <td>PD1 (6p)</td>
  </tr>
  <tr>
    <td rowspan="2"></td>
    <td rowspan="2">5</td>
    <td>2</td>
    <td>projekt</td>
    <td>P1: Prosta eksploracja danych, pomysły na wykresy, prototypy wykresów</td>
    <td>P1 (2p)</td>
  </tr>
  <tr>
    <td>2</td>
    <td>lab</td>
    <td>R: ggplot2 - theme, panele, mapy</td>
    <td></td>
  </tr>
  <tr>
    <td rowspan="2"></td>
    <td rowspan="2">6</td>
    <td>2</td>
    <td>wykład</td>
    <td>Nie popełniaj tych błędów</td>
    <td></td>
  </tr>
  <tr>
    <td>2</td>
    <td>lab</td>
    <td>R: ggplot2 - mapa Polski</td>
    <td>PD2 (6p)</td>
  </tr>
  <tr>
    <td rowspan="2"></td>
    <td rowspan="2">7</td>
    <td>2</td>
    <td>projekt</td>
    <td>P1: gotowe wykresy, projekt plakatu</td>
    <td>P1 (2p)</td>
  </tr>
  <tr>
    <td>2</td>
    <td>lab</td>
    <td>R: patchwork, ggrepel</td>
    <td></td>
  </tr>
  <tr>
    <td rowspan="3"></td>
    <td rowspan="3">8</td>
    <td>1</td>
    <td>wykład</td>
    <td rowspan="2">Oddanie P1</td>
    <td rowspan="2">P1 (20p)</td>
  </tr>
  <tr>
    <td>1</td>
    <td>projekt</td>
  </tr>
  <tr>
    <td>2</td>
    <td>lab</td>
    <td>R: plotly, leaflet</td>
    <td></td>
  </tr>
  <tr>
    <td rowspan="2"></td>
    <td rowspan="2">9</td>
    <td>2</td>
    <td>wykład</td>
    <td>Wizualizacja grafów, wizualizacja tekstu</td>
    <td></td>
  </tr>
  <tr>
    <td>2</td>
    <td>lab</td>
    <td>R: grafy, RMarkdown: generowanie raportów z analizą danych</td>
    <td>PD3 (6p)</td>
  </tr>
  <tr>
    <td rowspan="2"></td>
    <td rowspan="2">10</td>
    <td>2</td>
    <td>projekt</td>
    <td>P2: pomysł, źródło danych</td>
    <td>P2 (1p)</td>
  </tr>
  <tr>
    <td>2</td>
    <td>lab</td>
    <td>R: Shiny 1</td>
    <td></td>
  </tr>
  <tr>
    <td rowspan="2"></td>
    <td rowspan="2">11</td>
    <td>2</td>
    <td>wykład</td>
    <td>Dashboardy, historia, IBCS</td>
    <td></td>
  </tr>
  <tr>
    <td>2</td>
    <td>lab</td>
    <td>R: Shiny 2</td>
    <td>PD4 (6p)</td>
  </tr>
  <tr>
    <td rowspan="2"></td>
    <td rowspan="2">12</td>
    <td>2</td>
    <td>projekt</td>
    <td>P2: eksploracja danych, pomysły na wykresy, prototypy wykresów</td>
    <td>P2 (2p)</td>
  </tr>
  <tr>
    <td>2</td>
    <td>lab</td>
    <td>Python: pandas, numpy</td>
    <td></td>
  </tr>
  <tr>
    <td rowspan="3"></td>
    <td rowspan="3">13</td>
    <td>1</td>
    <td>projekt</td>
    <td>Konsultacje</td>
    <td></td>
  </tr>
  <tr>
    <td>1</td>
    <td>wykład</td>
    <td>Test</td>
    <td>Test (10p)</td>
  </tr>
  <tr>
    <td>2</td>
    <td>lab</td>
    <td>Python: matplotlib 1</td>
    <td>PD5 (6p)</td>
  </tr>
  <tr>
    <td rowspan="2"></td>
    <td rowspan="2">14</td>
    <td>2</td>
    <td>projekt</td>
    <td>P2: gotowe wykresy, prototyp wyglądu aplikacji</td>
    <td>P2 (2p)</td>
  </tr>
  <tr>
    <td>2</td>
    <td>lab</td>
    <td>Python: matplotlib 2, seaborn</td>
    <td></td>
  </tr>
  <tr>
    <td rowspan="3"></td>
    <td rowspan="3">15</td>
    <td>1</td>
    <td>wykład</td>
    <td rowspan="2">Oddanie P2</td>
    <td rowspan="2">P2 (24p)</td>
  </tr>
  <tr>
    <td>1</td>
    <td>projekt</td>
  </tr>
  <tr>
    <td>2</td>
    <td>lab</td>
    <td>Python: plotly, EDA</td>
    <td>PD6 (6p)</td>
  </tr>
</tbody></table>

### Schemat oceniania (suma 100p):

- projekt I (25p):

- projekt II (29p):

- prace domowe (36p = 6 x 6p)
	- za każdą pracę domową uzyskuje się do 6p

- test (10p)
    
    
Z każdego projektu należy uzyskać ponad 50% możliwych punktów.

| Ocena |  | 3 | 3.5 | 4 | 4.5 | 5 |
|:---:| :---: |:---:|:---:|:---:|:---:|:---:|
| Punkty |  | (50, 60] | (60, 70] | (70, 80] | (80, 90] | (90, ∞) |

